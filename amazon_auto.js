    ...
    try {
        ...
        const productData = await page.evaluate(() => {
            ...
            return {
                ...
                ratings:
                    parseFloat(
                        getText("#acrCustomerReviewText").replace(",", ".")
                    ) || null,
                ...
                details: {
                    brand: getText(".po-brand .a-size-base.po-break-word"),
                    color: getText(".po-color_name .a-size-base.po-break-word"),
                    size: getText(".po-size_name .a-size-base.po-break-word"),
                    material: getText(
                        ".po-fabric_type .a-size-base.po-break-word"
                    ),
                    fitType: getText(".po-fit_type .a-size-base.po-break-word"),
                    careInstructions: getText(
                        ".po-care_instructions .a-size-base.po-break-word"
                    ),
                },
            };
        });
        ...
        if (!productData.name || productData.name === "N/A") {
            console.log(
                `Warning: Product data for ${productId} is incomplete.`
            );
        }
...
const fetchProductsForCategory = async (page, category, searchUrl) => {
    ...
    if (fs.existsSync(filePath)) {
        console.log(
            `Data for category ${category} already exists. Skipping...`
        );
        return;
    }
